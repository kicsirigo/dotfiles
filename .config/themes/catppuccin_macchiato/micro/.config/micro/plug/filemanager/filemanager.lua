...
-- Find if the file is already open, and if so, activate its pane and return true
local function focus_file_if_open(abspath)
	local tabs = micro.Tabs()
	for i = 1, #tabs.List do
		local tab = tabs.List[i]
		for j = 1, #tab.Panes do
			local pane = tab.Panes[j]
			if pane.Buf ~= nil and pane.Buf.AbsPath ~= nil and pane.Buf.AbsPath == abspath then
				-- Switch to tab i (0-based in micro API, so i - 1)
				tabs:SetActive(i - 1)
				-- Switch to pane j (0-based in micro API, so j - 1)
				tab:SetActive(j - 1)
				return true
			end
		end
...
local function try_open_at_y(y)
	...
	elseif y > 2 and not scanlist_is_empty() then
		...
		if scanlist[y].dirmsg ~= "" then
			...
			-- Check if the file is already open in one of the panes
			if not focus_file_if_open(scanlist[y].abspath) then
...
-- Enter key handler mapped globally in bindings.json
function onEnterPressed(bp)
